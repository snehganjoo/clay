    ...
    rows: [
      {
        cells: [
          {
            description: 'Callout / Neutral',
            'img-source': '/images/callout/callout-default-neutral.png',
          },
          {
            description: 'Callout / Success',
            'img-source': '/images/callout/callout-default-neutral.png',
          },
          {
...
function CalloutVariantSectionContentCard({
  children,
  className,
  content,
  ...props
...
function CalloutVariantSection({ children, className, ...props }) {
  return (
    <>
      {calloutVariantContent.map((content) => (
        <CalloutVariantSectionContentCard
          key={content.heading}
          content={content}
        />
      ))}
...
function CalloutUsageSection({ children, className, ...props }) {}
function CalloutExampleSection({ children, className, ...props }) {}
...
export default async function Page() {
  return (
    <div className="">
      ...
      <H2>Callout</H2>
      <SubHeading>
        Callout draw attention to information of special importance including
      ...
      <Flowbite style={tabItemTheme}>
        <Tabs aria-label="Tabs with underline">
          <Tabs.Item active={tabs[0].current} title={tabs[0].title}>
            <CalloutVariantSection /*content={props.content}*/ />
          </Tabs.Item>
